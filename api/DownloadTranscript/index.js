...
module.exports = async function (context, req) {
  ...
  let videoId = videoIdOrUrl.match(/v=([^&]+)/) || videoIdOrUrl.match(/youtu.be\/([^&]+)/) || videoIdOrUrl;
  ...
  const result = await fetch(`https://youtube.googleapis.com/youtube/v3/captions?part=id&videoId=${videoId}&key=${process.env.YOUTUBE_API_KEY}`, {
    headers: {
      headers: {
  ...
  for (let i = 0; i < data.items.length; i++) {
    ...
    const trackResult = await fetch(`https://youtube.googleapis.com/youtube/v3/captions/${track.id}?tfmt=srt&key=${process.env.YOUTUBE_API_KEY}`, {
      headers: {
        'Authorization': 'Bearer ' + accessToken,
        'Accept': 'application/json'
      }
